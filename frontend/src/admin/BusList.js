...
const BusList = () => {
  ...
  const fetchBuses = async () => {
    try {
      const response = await axios.get("https://i-bus-e-ticket-1.onrender.com/api/bus/all"); 
  ...
  const deleteBus = async (id) => {
    if (window.confirm("Are you sure you want to delete this bus?")) {
      try {
        await axios.delete(`https://i-bus-e-ticket-1.onrender.com/api/bus/delete/${id}`);
  ...
  const saveBus = async (id) => {
    try {
      const response = await axios.put(`https://i-bus-e-ticket-1.onrender.com/api/bus/update/${id}`, updatedData);
